...
const TransferButton = ({ task }: TransferButtonProps) => {
  ...
  const [disableTransferButtonForTask, setDisableTransferButtonForTask] =
    useState(false);
  const [taskSidsTransferred, setTaskSidsTransferred] = useState<string[]>([]);

  // if there is a transfer task event for this chat disable the transfer button
  const handleTransferInitiated = (payload: any) => {
    setTaskSidsTransferred([...taskSidsTransferred, task.sid]);
  };
  ...
  useEffect(() => {
    setDisableTransferButtonForTask(taskSidsTransferred.includes(task.sid));
  }, [task.sid, taskSidsTransferred]);
