...
import TaskService from "../helpers/TaskRouterService";
...
class ChatTransferService extends ApiService {
  ...
  async executeChatTransfer(
    ...
    try {
      ...
      const {
        ...
        message,
      } = await this._createTransferTask(task, transferTargetSid, queueName);
      if (success) {
  ...
  async completeTransferredTask(task: Flex.ITask): Promise<Boolean> {
    try {
      ...
      const { success } = await this._completeTransferredTask(
        taskSid,
        transferType,
        channelSid
      );
  ...
  _createTransferTask = (
    ...
    return this.fetchJsonWithReject<CreateTransferTaskResponse>(
      `${this.baseUrl}/chat-transfer/flex/create-transfer-task`,
      {
        method: "post",
  ...
  _completeTransferredTask = (
    ...
    return this.fetchJsonWithReject<CompleteTransferredTaskResponse>(
      `${this.baseUrl}/features/chat-transfer/flex/complete-task-for-transfer`,
      {
        method: "post",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: this.buildBody(encodedParams),
